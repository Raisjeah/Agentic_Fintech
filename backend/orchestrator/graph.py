...
async def planner_node(state: TradingState):
    plan_out = planner_agent.create_plan(state["asset"], state["timeframe"], state["goal"], state["capital"], state["risk_percent"])
...
async def risk_node(state: TradingState):
    ...
    risk_out = risk_agent.evaluate_risk(state["asset"], state["market_data"]["ohlcv"], state["macro_data"], state["news_data"])
    return {"risk_data": risk_out}

async def thesis_node(state: TradingState):
    thesis_out = thesis_agent.generate_thesis(
...
async def synthesizer_node(state: TradingState):
    synth_out = synthesizer_agent.synthesize(
...
async def scenario_node(state: TradingState):
    scen_out = scenario_agent.generate_scenarios(
...
async def reviewer_node(state: TradingState):
    ...
    review_out = reviewer_agent.review_plan(synth_output, thesis_data, risk_data, mm_data)
...
def build_graph():
    ...
    # Risk needs data, macro, news. LangGraph doesn't do multiple-in well natively without fan-in logic.
    # To handle fan-in, we just make them flow sequentially for the graph MVP or use parallel execution 
    # where the next node waits for all to be populated.
    # Let's string them linearly for simplicity in this implementation, 
    # since `run_analysis` already runs them concurrently via asyncio in a better way.
    # Actually, let's use the power of LangGraph. We can route from data, news, macro to risk if we use a fan-in node, 
    # but the simplest way is to chain them. Let's fix the graph edges to be linear to avoid complex state management for MVP.
    pass

# For Phase 3, we will use a simpler linear flow in LangGraph to ensure stability
def build_linear_graph():
    ...
    workflow.add_edge("macro", "tech")
    workflow.add_edge("tech", "sentiment")
    workflow.add_edge("sentiment", "risk")
    workflow.add_edge("risk", "thesis")
...
trading_graph = build_linear_graph()
