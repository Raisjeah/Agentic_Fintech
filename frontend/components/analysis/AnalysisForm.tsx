...
export default function AnalysisForm({ asset = "BTC/USDT", onTrigger, isAnalyzing, progress }: any) {
  ...
  return (
    <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-lg mx-4 mt-4 mb-4 p-4 flex flex-col md:flex-row md:items-center gap-4 md:gap-0 md:space-x-6">
      {/* Asset Display */}
      <div className="flex flex-col md:border-r border-[var(--border-subtle)] md:pr-6 pb-2 md:pb-0 border-b md:border-b-0">
      ...
      <div className="flex flex-row md:flex-row items-center gap-4 md:space-x-6 w-full md:w-auto">
        {/* Goal Selector */}
        <div className="flex flex-col flex-1 md:flex-none">
          ...
          <select 
            ...
            className="w-full md:w-auto bg-[var(--bg-input)] border border-[var(--border-default)] rounded px-3 py-1.5 text-sm text-[var(--text-primary)] focus:outline-none focus:border-[var(--cyan-primary)]"
          >
        ...
        <div className="flex flex-col flex-1 md:flex-none">
          ...
          <div className="relative">
            ...
            <input 
              ...
              className="w-full md:w-32 bg-[var(--bg-input)] border border-[var(--border-default)] rounded px-3 py-1.5 pl-7 text-sm font-mono text-[var(--text-primary)] focus:outline-none focus:border-[var(--cyan-primary)]"
            />
          </div>
        </div>
      </div>
      ...
      {/* Risk Slider */}
      <div className="flex flex-col w-full md:flex-1 md:max-w-[200px] pt-2 md:pt-0">
      ...
      {/* Trigger Button */}
      <div className="flex-1 flex md:justify-end mt-2 md:mt-0">
        <button 
          onClick={() => onTrigger && onTrigger(goal, capital, risk)}
          disabled={isAnalyzing}
          className={`w-full md:w-auto font-bold px-6 py-3 md:py-2 rounded-md flex items-center justify-center transition-opacity ${
            isAnalyzing 
